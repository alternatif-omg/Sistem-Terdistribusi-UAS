...
function getRegistrations() {
    $db = getConnection();
    $sql = "SELECT registrations.*, students.name AS student_name, Activities.name AS activity_name 
            FROM registrations 
...
function createRegistration() {
    $data = json_decode(file_get_contents('php://input'), true);
    if (empty($data['student_id']) || empty($data['activity_id']) || empty($data['registration_date']) || empty($data['position'])) {
        header("HTTP/1.1 400 Bad Request");
        echo json_encode(["message" => "All fields are required"]);
        return;
    }
    ...
    $db = getConnection();
    $sql = "INSERT INTO registrations (student_id, activity_id, registration_date, position) VALUES (?, ?, ?, ?)";
    $stmt = $db->prepare($sql);
    try {
        $stmt->execute([$data['student_id'], $data['activity_id'], $data['registration_date'], $data['position']]);
...
function updateRegistration() {
    ...
    $db = getConnection();
    $sql = "UPDATE registrations SET student_id = ?, activity_id = ?, registration_date = ?, position = ? WHERE registration_id = ?";
    $stmt = $db->prepare($sql);
    try {
        $stmt->execute([$data['student_id'], $data['activity_id'], $data['registration_date'], $data['position'], $_GET['id']]);
...
function getAttendance() {
    $db = getConnection();
    $sql = "SELECT attendance.*, students.name AS student_name, Activities.name AS activity_name 
            FROM attendance 
...
function createAttendance() {
    $data = json_decode(file_get_contents('php://input'), true);
    if (empty($data['student_id']) || empty($data['activity_id']) || empty($data['attendance_date']) || empty($data['status'])) {
        header("HTTP/1.1 400 Bad Request");
        echo json_encode(["message" => "All fields are required"]);
        return;
    }
    ...
    $db = getConnection();
    $sql = "INSERT INTO attendance (student_id, activity_id, attendance_date, status) VALUES (?, ?, ?, ?)";
    $stmt = $db->prepare($sql);
    try {
        $stmt->execute([$data['student_id'], $data['activity_id'], $data['attendance_date'], $data['status']]);
...
function updateAttendance() {
    ...
    $db = getConnection();
    $sql = "UPDATE attendance SET student_id = ?, activity_id = ?, attendance_date = ?, status = ? WHERE attendance_id = ?";
    $stmt = $db->prepare($sql);
    try {
        $stmt->execute([$data['student_id'], $data['activity_id'], $data['attendance_date'], $data['status'], $_GET['id']]);
